...
public class BankQuestionDaoImpl implements BankQuestionDao {
    ...
    public List<BankQuestion> findAll() {
        ...
        List<BankShortAnswerQuestion> list4 = (List<BankShortAnswerQuestion>) (session.createQuery("from BankShortAnswerQuestion").list());
    ...
    @Override
    @Transactional(propagation = Propagation.NOT_SUPPORTED)
    public BankShortAnswerQuestion findShortAnswerById(int id) {
        return sessionFactory.getCurrentSession().get(BankShortAnswerQuestion.class, id);
    }
    ...
    @Override
    public int countAllShortAnswer() {
        Object obj = sessionFactory.getCurrentSession().createQuery("select count(*) from BankShortAnswerQuestion").uniqueResult();
        return ((Number) obj).intValue();
    }
    ...
    @Override
    public List<BankShortAnswerQuestion> findAllShortAnswer() {
        @SuppressWarnings("unchecked")
        List<BankShortAnswerQuestion> list = (List<BankShortAnswerQuestion>) (sessionFactory.getCurrentSession().createQuery("from BankShortAnswerQuestion").list());
        return list;
    }
    ...
    @Override
    public List<BankShortAnswerQuestion> findShortAnswerWithComposeFlag(int composeFlag) {
        Query q = sessionFactory.getCurrentSession().createQuery("from BankShortAnswerQuestion where composeFlag=:c0");
        q.setInteger("c0", composeFlag);
        @SuppressWarnings("unchecked")
        List<BankShortAnswerQuestion> list = (List<BankShortAnswerQuestion>) (q.list());
        return list;
    }
    ...
    @Override
    public List<BankShortAnswerQuestion> findAllShortAnswerWithPage(int pageIndex, int pageSize) {
        Query q = sessionFactory.getCurrentSession().createQuery("from BankShortAnswerQuestion");
        int firstResult = pageIndex * pageSize;
        q.setFirstResult(firstResult);
        q.setMaxResults(pageSize);
        @SuppressWarnings("unchecked")
    ...
    @Override
    public List<BankShortAnswerQuestion> findShortAnswerForSearch(String content, String answer, String knowledge) {
        StringBuffer qStrSb = new StringBuffer("from BankShortAnswerQuestion where content like :c0 ");
        boolean answerNotNull = false;
        if (!answer.equals("")) {
            qStrSb.append(" and (answer like :a) ");
            answerNotNull = true;
        ...
        @SuppressWarnings("unchecked")
        List<BankShortAnswerQuestion> list = (List<BankShortAnswerQuestion>) query.list();
        return list;
    }
    ...
    private enum ImportStringLineTypeForChoice {
        CONTENT, OPTION_A, OPTION_B, OPTION_C, OPTION_D, OPTION_E, OPTION_F,
        OPTION_G, OPTION_H, ANSWER, KNOWLEDGE_POINT
    }
...
//						logger.debug("第二行："+line);
    ...
    @Override
    @Transactional(propagation = Propagation.NOT_SUPPORTED)
    public int importShortAnswerFromTxt(File txtFile) {
        BufferedReader br = null;
        int cnt = 0;
        try {
            br = new BufferedReader(new InputStreamReader(new FileInputStream(txtFile), "UTF-8"));
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (line.equals("")) {
                    ...
                    if (content == null || content.equals("")) {
                        ...
                        BankShortAnswerQuestion q = null;
                        if (answer != null && !answer.equals("")) {
                            q = new BankShortAnswerQuestion(content, answer, knowledgePoint);
                        }
